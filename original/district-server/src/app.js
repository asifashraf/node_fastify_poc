...
const { ApolloServerPluginUsageReporting, ApolloServerPluginDrainHttpServer, ApolloServerPluginLandingPageDisabled } = require('apollo-server-core');
...
//const firebaseAppChecker = require('./lib/middlewares/firebase-app-checker')(firebase.admin);
...
const getApp = async (
  ...
  if (!isProd && process.env.APOLLO_KEY) {
    apolloSettings.plugins.push(
      ApolloServerPluginUsageReporting({
        sendVariableValues: { all: true },
        sendHeaders: { all: true },
      }));
  }
  const graphQLServer = new ApolloServer(apolloSettings);
  ...
  texpressConsumer(queryContextWithoutAuth);
  barqConsumer(queryContextWithoutAuth);
...
const getDeviceMetadata = async ctx => {
  try {
      ...
      if (customerId) {
        ...
        if (deviceId) {
          ...
          if (customerId && deviceId && deviceIdentifierType) {
            await customerDeviceMetadataAssociation(
              ctx,
              customerId,
    ...
    console.log('getDeviceMetadata-error', { stack, message });
  }
};
