...
knex
  .transaction(async trx => {
    const query = `
    select * from wallet_accounts wa where referral_amount > 0 or referral_amount_expires_on > 0; 
`;
    ...
    const data = map(
      ...
      n => {
        return {
          ...
          amount: n.referralAmount,
          expires_on: n.referralAmountExpiresOn,
        };
      }
    );
    ...
    if (data && Array.isArray(data) && data.length > 0) {
      ...
      const newEntries = map(data, d => {
        return trx('wallet_account_referrals').insert(d);
      });
