...
const { paymentStatusName, orderSetSource, orderCouponTypes, couponDetailUsedOn } = require('../root/enums');
...
module.exports = {
  CSubscriptionOrder: {
    ...
    async usedCouponDetail({id, couponId}, args, context) {
      if (couponId) {
        const couponDetails = await context.usedCouponDetail.getAllUsedOn(
          couponDetailUsedOn.SUBSCRIPTION_ORDER,
          id
        );
        return couponDetails?.length > 0 ? couponDetails[0] : null;
      }
      return null;
  ...
  Query: {
    ...
    async checkSubscriptionCoupon(root, args, context) {
      ...
      const isCouponValid = await context.coupon.isCouponCodeValid(args.couponInput.code);
      if (!isCouponValid) {
        const country = await context.country.getByCode(args.couponInput.countryIso);
        const invalidCouponAttemp = {
          customerId: context.auth.id,
          couponCode: args.couponInput.code,
          ...(country && { countryId: country.id }),
          ...(args.couponInput.brandId && { brandId: args.couponInput.brandId }),
          orderType: orderCouponTypes.SUBSCRIPTION_ORDER
        };
        await this.context.invalidCouponAttemps.save(invalidCouponAttemp);
      }
