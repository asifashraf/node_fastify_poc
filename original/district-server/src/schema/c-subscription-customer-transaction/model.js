...
class CSubscriptionCustomerTransaction extends BaseModel {
  ...
  async getUsedCupNumbers(subscriptionCustomerIds) {
    let cupUsageBySubscriptionCustomerIds = [];
    await Promise.all(subscriptionCustomerIds.map(async (id) => {
      const start = await this.roDb(this.tableName)
        .select(this.db.raw('remaining_cups'))
        .where('action_type', cSubscriptionActionType.STARTED)
        .where('subscription_customer_id', id)
        .first();
      const lastOrder = await this.roDb(this.tableName)
        .select(this.db.raw('remaining_cups'))
        .where('action_type', cSubscriptionActionType.ORDER_PLACED)
        .where('subscription_customer_id', id)
        .orderBy('created', 'desc')
        .limit(1);
      cupUsageBySubscriptionCustomerIds.push({
        start: start.remainingCups,
        id,
        usedCups: (lastOrder.length > 0) ? start.remainingCups - lastOrder[0].remainingCups : 0
      });
    }));
    cupUsageBySubscriptionCustomerIds = cupUsageBySubscriptionCustomerIds.map(row => {
      return {
        subscriptionCustomerId: row.id,
        count: row.usedCups
      };
    });
    return cupUsageBySubscriptionCustomerIds;
    // return this.roDb(this.tableName)
