...
class MenuItem extends BaseModel {
  ...
  async save(menuItems) {
    ...
    const saveMenuItem = async menuItem => {
      ...
      options = removeLocalizationField(options, 'value');
    ...
    if (args) {
      if (args.filters) {
        if (args.filters?.searchText) {
          ...
          if (brandLocationId) {
            const supportedLanguagesAndKeys = [
              { lang: 'en', key: 'name.en'},
              { lang: 'ar', key: 'name.ar'},
              { lang: 'tr', key: 'name.tr'}
            ];
            ...
            const options = {
              ...
              includeScore: true,
              threshold: 0.3,
              keys: supportedLanguagesAndKeys.map(t => t.key),
            };
            ...
            results.forEach(result => {
              result.matches.forEach(resMatch => {
                const { key } = resMatch;
                const existMapping = supportedLanguagesAndKeys.find(t => t.key === key);
                const isConflicted = existMapping.lang !== lang;
                if (isConflicted) {
                  resMatch.indices = [[0, 0]];
                }
              });
            });
            let pageVal = 1;
