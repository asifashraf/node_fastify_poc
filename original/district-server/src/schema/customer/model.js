...
class Customer extends BaseModel {
  ...
  update(customer) {
    ...
    return super.save(omit(customer, ['notificationSettings']));
  }
  ...
  async validateAndRegister(customer) {
    ...
    try {
      ...
      if (existCustomer) {
        ...
        await this.db(this.tableName).insert(customer);
        return errors;
      }
    } catch (err) {
  ...
  async getLastPaymentMethod({ id, currencyId }) {
    const lastPaymentMethod = await this.db('order_sets as os')
      .select('opm.payment_method as payment_method')
      .leftJoin('order_payment_methods as opm', 'opm.reference_order_id', 'os.id')
      .where('customer_id', id)
      .andWhere('currency_id', currencyId)
      .orderBy('created_at', 'desc')
      .first();
    console.log(lastPaymentMethod);
    return lastPaymentMethod?.paymentMethod;
  }
...
`).then(result => transformToCamelCase(result.rows));
  ...
  async sendPurchaseEvent(orderSet, orderItems, brand) {
    ...
    // Note: Normally there is SQS solution, but we are changing the mechanism
    // that's why I added this SNS solution
    publishEvent(Topic.ANALYTICS_EVENTS,
      {
        eventType: customerAnalyticsEvents.ORDER_PURCHASE,
