...
class PaymentGatewayCharge extends BaseModel {
  ...
  async findByPaymentGatewayAndMethodWithCountryId(paymentGateway, paymentMethod, countryId) {
    return this.db(this.tableName)
      .where('payment_gateway', paymentGateway)
      .where('payment_method', paymentMethod)
  ...
  async deletePaymentGatewayCharge(id) {
    const deletedPGC = Boolean(await super.getById(id));
    if (deletedPGC) {
      await this.db
