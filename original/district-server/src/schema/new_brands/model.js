...
class NewBrands extends BaseModel {
  ...
  async validate(countryId, newBrands) {
    const errors = [];
    ...
    const countryBrands = await this.context.brand
      .selectFields(['id'])
      .where('country_id', countryId) || [];

    for (const { brandId } of newBrands) {
      const brand = countryBrands.find(brand => brand.id === brandId);
      if (!brand) {
        errors.push(newBrandsSaveError.WRONG_COUNTRY);
        break;
      }
    }
    return errors;
