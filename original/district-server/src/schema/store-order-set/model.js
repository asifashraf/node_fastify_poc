...
const { paymentSchemes, paymentMethodType } = require('../../payment-service/enums');
const {
  notifications: {
...
class StoreOrderSet extends BaseModel {
  ...
  filterStoreOrderSets(query, filters) {
    ...
    if (filters.storeOrderId) {
      query.where('store_order_set_statuses.id', filters.storeOrderId);
    }
  ...
  async storeOrderSetStatusTotal(filters) {
    const cloneFilters = Object.assign({}, filters);
    delete cloneFilters.statuses;
    const data = {
      placed: 0,
    ...
    const query = this.getAll(cloneFilters);
    query.clearSelect();
    query.clearOrder();
    query
      .select(
  ...
  async storeOrderSetCreate(input) {
    ...
    if (isNullOrUndefined(country)) {
      country =
        await this.context.customerAddress.getCountryFromCountryCodeByAddressId(
          input.customerAddressId
        );
    }
    const { paymentProvider, customerCardToken } =
    ...
    const { products, currency, components, shipping } =
  ...
  */
  ...
  async orderPricingCalculations(input) {
    ...
    if (useCredits) {
      const customerBalance =
        await this.context.loyaltyTransaction.getBalanceByCustomer(
          customerId,
          currency.id
        );
  ...
  async calculateShipping(input, calculation) {
    ...
    const pickupLocations =
      await this.context.product.getPickupLocationsWithDeliveryAddressDistance(
        productIds,
        customerAddressId
      );
    ...
    let deliveryFee = 1;
    const deliveryEstimate = country.storeOrderDeliveryTime
      ? country.storeOrderDeliveryTime * 24
      : 24;
    if (
  ...
  async getAvailablePaymentMethods({
    ...
    const providerPaymentMethods =
      await this.context.paymentService.getPaymentMethods({
        countryCode,
        currencyCode,
        customerId,
        amount,
...
`;
  ...
  async getPaymentMethod({ id }) {
    const orderPaymentMethod =
      await this.context.orderPaymentMethod.getStoreOrderSetPaymentMethod(id);
    let paymentMethod =
  ...
  async getPaymentMethodLabel(storeOrderSet) {
    ...
    const paymentMethodPromise = this.getPaymentMethod({
      id: storeOrderSet.id,
    });

    const [computeInvoiceComponents, paymentMethod] = await Promise.all([
      computeInvoiceComponentsPromise,
    ...
    const isCredits = find(
      computeInvoiceComponents.components,
      t => t.type === orderPaymentMethods.CREDITS
    );
    if (isCredits) methods.push(orderPaymentMethods.CREDITS);

    const isGiftCard = find(
      computeInvoiceComponents.components,
      t => t.type === orderPaymentMethods.GIFT_CARD
    );
  ...
  async getStoreOrderSetExporting(filters) {
    const dateRange = filters.dateRange;
    let endDate = moment();
    const todayEnd = moment().endOf('day');
    if (get(dateRange, 'endDate')) {
      endDate = get(dateRange, 'endDate').endOf('day');
      if (endDate.isAfter(todayEnd)) endDate = todayEnd;
    } else endDate = todayEnd;
    filters.dateRange = {
      startDate: endDate.clone().subtract(1, 'w'),
      endDate
    };
    const items = await this.getAll(filters);
    return { items };
  }
  async getPaymentMethodsForStoreOrderSet(id) {
    const storeOrderSet = await this.getById(id);
    const invoice = await this.computeInvoice(storeOrderSet);
    ...
    if (paymentMethod) {
      const totalAmount = filter(invoice.components, t => t.type === invoiceComponentType.TOTAL);
      let paidAmount = totalAmount[0].value;
      if (isCredits.length == 1) {
        paidAmount = paidAmount - isCredits[0].value;
      }
      methods.push({
        name: paymentMethod.paymentScheme,
        imageUrl: paymentMethod.imageUrl ? paymentMethod.imageUrl : null,
        subInfo: paymentMethod.subText ? paymentMethod.subText : null,
        amount: paidAmount,
    ...
    const localizedList = map(methods, t => {
      t.name = this.getLocalizedMethodTitle(t.name);
      return t;
    });
