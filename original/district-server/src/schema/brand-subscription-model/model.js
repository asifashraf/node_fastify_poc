...
class BrandSubscriptionModel extends BaseModel {
  ...
  async validate({ brandId, hasTrial, signDate, expiryDate, trialStartDate, trialEndDate }) {
    const errors = [];
    const errorDescription = null;
    ...
    if (moment(expiryDate).isBefore(moment())) {
      errors.push(brandSubscriptionModelError.INVALID_EXPIRY_DATE);
    }
    /*
    if(revenueModel === revenueModel.FREE_TRIAL_MODEL && autoRenewal){
      errors.push(brandSubscriptionModelError.FREE_TRIAL_AUTO_RENEWAL_ERROR);
    }
    */
    if (hasTrial) {
      if (trialStartDate && trialEndDate) {
        if (moment(trialEndDate).isAfter(moment(expiryDate))
          || moment(trialStartDate).isBefore(moment(signDate))) {
          errors.push(brandSubscriptionModelError.FREE_TRIAL_INVALID_DATE);
        }
      } else errors.push(brandSubscriptionModelError.FREE_TRIAL_UNDEFINED_DATE);
    }
    return { errors, errorDescription };
  }
  ...
  async addBrandSubscription(model) {
    ...
    if (brandSubscriptions.length > 0) {
      ...
      if (isSameModel) {
        ...
        SlackWebHookManager.sendTextToSlack(`[INFO] The brand subscription model (${bs.id}) has become inactive. New one (${latestId}) was generated.`);
      }
    } else {
      latestId = await this.save(model);
    }
  ...
  async checkTrialChaged(oldModel, newModel) {
    if (oldModel.hasTrial !== newModel.hasTrial) {
      const trialData = newModel.hasTrial
        ? { startDate: newModel.trialStartDate, endDate: newModel.trialEndDate}
        : { startDate: oldModel.trialStartDate, endDate: oldModel.trialEndDate};
      SlackWebHookManager.sendTextToSlack('[INFO]' + (newModel.hasTrial ? 'Trial added to ' : 'Trial removed from ') + ` Brand Subscription Model. ID: [${oldModel.id}] ` +
        (newModel.hasTrial ? 'Added Trial Data:' : 'Removed Trial Data:') + JSON.stringify(trialData)
      );
    } else if (newModel.hasTrial &&
        (moment(oldModel.trialEndDate).format('YYYY-MM-DD') !== moment(newModel.trialEndDate).format('YYYY-MM-DD') ||
        moment(oldModel.trialEndDate).format('YYYY-MM-DD') !== moment(newModel.trialEndDate).format('YYYY-MM-DD'))) {
      const trialData = {
        newStartDate: newModel.trialStartDate,
        newEndDate: newModel.trialEndDate,
        oldStartDate: oldModel.trialStartDate,
        oldEndDate: oldModel.trialEndDate};
      SlackWebHookManager.sendTextToSlack(`[INFO] Trial updated in Brand Subscription Model. ID: [${oldModel.id}] ` +
        'Changed Trial Data:' + JSON.stringify(trialData));
    }
  }
