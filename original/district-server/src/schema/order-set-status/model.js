...
const { MenuIntegrationServices } = require('../../lib/menu-integration-services/index');
// eslint-disable-next-line no-unused-vars
...
const { customerAnalyticsEvents } = require('../root/enums');
...
const { createEInvoiceForOrderSet } = require('../../../src/lib/e-invoice/e-invoice-trigger');
const menuIntegrationServices = require('../../lib/menu-integration-services/index');
class OrderSetStatus extends BaseModel {
  ...
  async isAllreadyCompleted(orderSetId) {
    const { count } = await this.db(this.tableName)
      .count('*')
      .where('order_set_id', orderSetId)
      .where('status', orderSetStatusNames.COMPLETED)
      .first();
    return parseInt(count, 10) >= 2;
  }
  async setStatusForOrderSetId(orderSetId, status, context, note) {
    ...
    if ((latestStatus && latestStatus.status !== status) || !latestStatus) {
      ...
      const rollbackIsPaidProcess =
        latestStatus &&
        latestStatus.status === orderSetStatusNames.REPORTED &&
        orderSet.paymentMethod === 'CASH' &&
        status !== orderSetStatusNames.COMPLETED &&
        orderSet.paid;
      if (rollbackIsPaidProcess) {
        let amountDue = Number(orderSet.total);
        if (orderSet.prePaid) {
          const prePaid = orderSet.prePaid;
          if (prePaid.creditsUsed && Number(prePaid.creditsUsed) > 0) {
            amountDue -= Number(prePaid.creditsUsed);
          }
          if (prePaid.giftCards && prePaid.giftCards.length > 0) {
            amountDue -= Number(prePaid.giftCards[0].value);
          }
          if (prePaid.discoveryCreditUsed && Number(prePaid.discoveryCreditUsed) > 0) {
            amountDue -= Number(prePaid.discoveryCreditUsed);
          }
        }
        if (amountDue > 0) {
          await context.paymentStatus.save({
            ...
            name: paymentStatusName.PAYMENT_PENDING,
            rawResponse: '{"isCash": true, "paid": false}',
            isCashPayment: true,
          });
          await context.orderSet.save({ id: orderSet.id, paid: false, amountDue });
        }
      }
      ...
      if (status === orderSetStatusNames.PLACED) {
        ...
        publishEvent(
          ...
          {
            eventType: customerAnalyticsEvents.TOTAL_ORDER_PLACED,
            orderType: paymentStatusOrderType.ORDER_SET,
            referenceOrderId: orderSetId,
            customerId: orderSet.customerId,
          },
        ...
        if (Number(count) === 1) {
          publishEvent(
            Topic.ANALYTICS_EVENTS,
            {
              eventType: customerAnalyticsEvents.FIRST_ORDER_PLACED,
              orderType: paymentStatusOrderType.ORDER_SET,
              referenceOrderId: orderSetId,
              customerId: orderSet.customerId,
            },
        ...
        const integratedBrands = await context.brand.getByBrandLocation(orderSet.brandLocationId);

        if (integratedBrands.enableFoodics) {
        ...
        if (integratedBrands.enableFoodics) {
          await menuIntegrationServices.otterServices.sendMessage(
            {
              orderSetId,
            },
            2
      ...
      if (status === orderSetStatusNames.COMPLETED) {
        ...
        /**
         * Order can be multiple time completed, Example after reporting
         * Adding Rewards and Referral process are ignored.
         */
        const ignoreProcess = orderSet.paymentMethod === 'CASH' && (await this.isAllreadyCompleted(orderSetId));
        if (!ignoreProcess) {
        ...
        if (subscriptionInfo) {
          ...
          if (subscriptionIds && subscriptionIds.length > 0) {
            for (const subscriptionId of subscriptionIds) {
              await context.cSubscriptionCustomerTransaction.finishSubscriptionIfNoUsageRemaining({
                customerId: orderSet.customerId,
                subscriptionId,
              });
            }
        ...
        await context.customer.sendPurchaseEvent(orderSet, orderSet.items, brand);
        ...
        publishEvent(
          Topic.ANALYTICS_EVENTS,
          {
            eventType: customerAnalyticsEvents.TOTAL_ORDER_COMPLETED,
            orderType: paymentStatusOrderType.ORDER_SET,
            referenceOrderId: orderSetId,
            customerId: orderSet.customerId,
          },
        ...
        if (Number(count) === 1) {
          publishEvent(
            Topic.ANALYTICS_EVENTS,
            {
              eventType: customerAnalyticsEvents.FIRST_ORDER_COMPLETED,
              orderType: paymentStatusOrderType.ORDER_SET,
              referenceOrderId: orderSetId,
              customerId: orderSet.customerId,
            },
        ...
        // REWARDS_REDEMPTION **********/
        const customerUsedPerks = await this.context.customerUsedPerk.getByOrderSetId(orderSet.id);
        ...
        // Send E-Invoice to related services
        createEInvoiceForOrderSet({
          ctx: this.context,
          orderSet,
          brand,
          brandLocation,
      ...
      if (status === orderSetStatusNames.ACCEPTED) {
        ...
        if (
          arrivingInfo &&
          anHourAgo.isSameOrBefore(moment(arrivingInfo.arrivalTime)) &&
      ...
      const notifs = await notificationsForStatusChange(orderSetId, status, context);
    ...
    if (orderFulfillment && orderFulfillment.type === orderTypes.DELIVERY) {
      ...
      if (
        ...
        try {
          ...
          if (deliveryPartner === deliveryPartners.CAREEM || deliveryPartner === deliveryPartners.MASHKOR) {
            fullDeliveryAddress.unitNumber = fullDeliveryAddress.dynamicData[addressFieldType.UNIT_NUMBER]
              ? fullDeliveryAddress.dynamicData[addressFieldType.UNIT_NUMBER]
              : fullDeliveryAddress.unitNumber;
            fullDeliveryAddress.floor = fullDeliveryAddress.dynamicData[addressFieldType.FLOOR]
              ? fullDeliveryAddress.dynamicData[addressFieldType.FLOOR]
              : fullDeliveryAddress.floor;
            fullDeliveryAddress.buildingName = fullDeliveryAddress.dynamicData[addressFieldType.BUILDING_NAME]
              ? fullDeliveryAddress.dynamicData[addressFieldType.BUILDING_NAME]
              : fullDeliveryAddress.buildingName;
            fullDeliveryAddress.block = fullDeliveryAddress.dynamicData[addressFieldType.BLOCK]
              ? fullDeliveryAddress.dynamicData[addressFieldType.BLOCK]
              : fullDeliveryAddress.block;
            fullDeliveryAddress.area = fullDeliveryAddress.dynamicData[addressFieldType.AREA];
            fullDeliveryAddress.street = fullDeliveryAddress.dynamicData[addressFieldType.STREET]
              ? fullDeliveryAddress.dynamicData[addressFieldType.STREET]
              : fullDeliveryAddress.street;
            fullDeliveryAddress.landmark = fullDeliveryAddress.dynamicData[addressFieldType.LANDMARK];
            fullDeliveryAddress.avenue = fullDeliveryAddress.dynamicData[addressFieldType.AVENUE];
          ...
          fullDeliveryAddress.line1 =
            (fullDeliveryAddress && fullDeliveryAddress.neighborhood ? fullDeliveryAddress.neighborhood + ', ' : '') +
            fullDeliveryAddress.line1;
          if (deliveryPartner === deliveryPartners.JEEBLY) {
            omittedFields.concat(['unitNumber', 'floor', 'buildingName', 'area', 'street']);
          }
          ...
          if (deliveryPartner === deliveryPartners.TALABAT || deliveryPartner === deliveryPartners.BARQ) {
          ...
          SlackWebHookManager.sendTextToSlack(
            `[ERROR] Can not send CREATE_NEW_ORDER request Delivery SQS for the order(${orderSetId}). Please check it.`
          );
        }
      }
    }
  ...
  async validate({ orderSetId, status, note }) {
    const errors = [];
    const { status: lastStatus, clientName, brandLocationId } = await this.getLatestByOrderSet(orderSetId);
    ...
    switch (status) {
      case orderSetStatusNames.PLACED:
        if (![orderSetStatusNames.INITIATED, orderSetStatusNames.REPORTED].includes(lastStatus)) {
      ...
      case orderSetStatusNames.READY_FOR_PICKUP:
        if (
          [orderTypes.EXPRESS_DELIVERY, orderTypes.DELIVERY].includes(orderFulfillment.type) ||
          ![orderSetStatusNames.PREPARING, orderSetStatusNames.PREPARED, orderSetStatusNames.REPORTED].includes(
            lastStatus
          )
        ) {
          errors.push(orderSetStatusError.UNEXPECTED_REQUESTED_STATUS);
      ...
      case orderSetStatusNames.WAITING_FOR_COURIER:
        if (
          orderFulfillment.type !== orderTypes.DELIVERY ||
          ![orderSetStatusNames.PREPARING, orderSetStatusNames.PREPARED, orderSetStatusNames.REPORTED].includes(
            lastStatus
          )
        ) {
      ...
      case orderSetStatusNames.DELIVERY_DELAYED:
        if (
          orderFulfillment.type !== orderTypes.EXPRESS_DELIVERY ||
          ![orderSetStatusNames.OUT_FOR_DELIVERY, orderSetStatusNames.REPORTED].includes(lastStatus)
        ) {
          errors.push(orderSetStatusError.UNEXPECTED_REQUESTED_STATUS);
        }
        break;
      case orderSetStatusNames.DELIVERED:
        if (
          ...
          ![
            ...
            orderSetStatusNames.DELIVERY_DELAYED,
            orderSetStatusNames.REPORTED,
          ].includes(lastStatus)
        ) {
          errors.push(orderSetStatusError.UNEXPECTED_REQUESTED_STATUS);
      ...
      case orderSetStatusNames.COMPLETED:
        if (
          ![orderSetStatusNames.READY_FOR_PICKUP, orderSetStatusNames.DELIVERED, orderSetStatusNames.REPORTED].includes(
            lastStatus
          )
        ) {
          errors.push(orderSetStatusError.UNEXPECTED_REQUESTED_STATUS);
    ...
    if (admin) {
      if (clientName && clientName === orderSetStatusPlatforms.BARISTA_MPOS) {
        const orderFulfillment = await this.context.orderFulfillment.getByOrderSet(orderSetId);
        if (orderFulfillment.type === fulfillmentType.DELIVERY) {
          ...
          const deliveryConfiguration = await this.context.countryConfiguration.getByKey(
            countryConfigurationKeys.AUTOMATIC_DELIVERY_INTEGRATION,
            brand.countryId
          );
          const isAutomaticDeliveryIntegrationActive =
            deliveryConfiguration &&
            deliveryConfiguration.configurationValue === 'true' &&
            brandLocation.isAutomaticDeliveryIntegrationActive;

          let autoDeliverySuccess = false;
          if (isAutomaticDeliveryIntegrationActive) {
            const deliveryOrder = await this.roDb('delivery_orders').select('*').orderBy('order_id', orderSetId);
            if (deliveryOrder && deliveryOrder.status === 'SUCCESS') {
              autoDeliverySuccess = true;
              errors.push(orderSetStatusError.ADMIN_PORTAL_CAN_NOT_UPDATE_MPOS_DELIVERY_ORDER);
            }
          }
          if (
            !autoDeliverySuccess &&
            [orderSetStatusNames.ACCEPTED, orderSetStatusNames.PREPARING].includes(lastStatus)
          ) {
            errors.push(orderSetStatusError.ADMIN_PORTAL_CAN_NOT_UPDATE_MPOS_DELIVERY_ORDER);
          }
        } else if (!(status === orderSetStatusNames.REPORTED && lastStatus === orderSetStatusNames.COMPLETED)) {
          errors.push(orderSetStatusError.ADMIN_PORTAL_CAN_NOT_UPDATE_MPOS_PICKUP_ORDER);
        }
      }
      //if (status === orderSetStatusNames.ACCEPTED) errors.push(orderSetStatusError.ADMIN_PORTAL_CAN_NOT_ACCEPT_ORDER);
      if (!this.context.auth.isVendorAdmin && !note) errors.push(orderSetStatusError.INVALID_STATUS_NOTE);
    }
    if (status === orderSetStatusNames.COMPLETED) {
      const statusHistory = await this.context.paymentStatus.getAllByOrderSetId(orderSetId);
  ...
  async validateBulkOrderSet({ orderSetIds, status }) {
    ...
    if (admin && !this.context.auth.isVendorAdmin) {
      ...
      const orderSets = await this.context
        .db(`${this.context.orderSet.tableName} as os`)
  ...
  async rejectOrder(orderSetId, rejectionInfo) {
    const isLastStatusRejected = await this.lastStatusValidation(orderSetId, orderSetStatusNames.REJECTED);
    ...
    const orderSet = await this.context.orderSet
      .selectFields(['short_code', 'is_cashback_coupon', 'paid', 'brand_location_id', 'coupon_id', 'customer_id'])
      .where('id', orderSetId)
      .then(first);
    if (!orderSet) {
      throw orderSetError.INVALID_ORDER_SET;
    }
    const orderFulfillmentType = await this.context.orderFulfillment.getFulfillmentTypeByOrderSet(orderSetId);
    if (orderFulfillmentType === fulfillmentType.DELIVERY) {
      const url = deliveryServiceUrl + '/api/delivery-order/cancel/' + orderSet.shortCode;
      const options = {
        headers: {
    ...
    if (orderSet.couponId && !(orderSet.isCashbackCoupon && orderSet.paid)) {
      await this.context.coupon.incrementCouponCountersForCustomer(orderSet.couponId, orderSet.customerId, -1);
    }
  ...
  async createRejectionForOrderSetId(orderSetId, rejectionInfo, context) {
    ...
    if (orderFulfillment && orderFulfillment.type === fulfillmentType.DELIVERY) {
      const url = deliveryServiceUrl + '/api/delivery-order/cancel/' + orderSet.shortCode;
      try {
        const options = {
          headers: {
    ...
    await this.context.withTransaction('customerUsedPerk', 'changeUsedPerksStatus', orderSetId, 0);
    const statusId = await this.save(
      {
        orderSetId,
        status: orderSetStatusNames.REJECTED,
    ...
    const notifs = await notificationsForStatusChange(orderSetId, orderSetStatusNames.REJECTED, context);
  ...
  async undoRejectionForOrderSetId(orderSetId) {
    ...
    if (latestStatus.status !== orderSetStatusNames.REJECTED) {
      return Promise.reject(new Error('Order is not in a rejected state, unable to undo rejection.'));
    }
    ...
    const latestNonRejectionStatus = await this.db(this.tableName)
      ...
      .limit(1)
      .then(first);
    const retainedData = pick(latestNonRejectionStatus, ['orderSetId', 'status']);
    // allow to redeem once more if cashback and order was not paid or it was non cashback promo
    if (orderSet && orderSet.isCashbackCoupon) {
      if (!orderSet.paid) {
        await this.context.orderSet.incrementCouponCountersForOrderSet(orderSetId, 1);
      }
    } else if (orderSet) {
      await this.context.orderSet.incrementCouponCountersForOrderSet(orderSetId, 1);
    }
    ...
    await this.context.withTransaction('customerUsedPerk', 'changeUsedPerksStatus', orderSetId, 1);
    ...
    if (!('clientName' in input)) {
      switch (context.req.clientPlatform) {
        ...
        case 'driver':
          ...
          } else input = { ...input, clientName: orderSetStatusPlatforms.DRIVER };
          break;
        case 'ios':
        case 'android':
