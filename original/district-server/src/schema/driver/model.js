...
const { getExpressDeliveryTrackingPageShortUrlForRider } = require('../../lib/dynamic-url');
...
${rtlAndLtrTextConversion('en', estimatedTimeStr)}`;
  ...
  async prepareSMSBodyV2({ driverLanguage, token, brand, branch, order, orderQueue }) {
    ...
    const { shortLink } = await getExpressDeliveryTrackingPageShortUrlForRider(riderUrl);
