...
const {createEInvoice} = require('../../../src/lib/e-invoice/e-invoice-trigger');
...
class LoyaltyOrder extends BaseModel {
  ...
  async prepareEInvoice({loyaltyOrderId}) {
    try {
      const loyaltyOrder = await this.context.loyaltyOrder.getById(loyaltyOrderId);
      const country = await this.context.country.getByCurrencyId(loyaltyOrder.currencyId);
      const components = [
        {
          type: 'TOTAL',
          value: loyaltyOrder.amount,
        },
        {
          type: 'AMOUNT_PAID',
          value: loyaltyOrder.amount
      ...
      createEInvoice({
        ctx: this.context,
        orderType: paymentStatusOrderType.LOYALTY_ORDER,
        orderId: loyaltyOrder.id,
        orderCreatedAt: loyaltyOrder.createdAt,
        orderShortCode: undefined,
        customerId: loyaltyOrder.customerId,
        countryId: country.id,
        items,
        components
