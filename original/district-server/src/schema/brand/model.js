...
class Brand extends BaseModel {
  ...
  async validate(brand) {
    ...
    forEach(result.flat(), r => {
      if (r) {
        if (r.brandLocationId) {
          errors.push(brandSaveError.BRANCH_ADMIN_CAN_NOT_BE_BRAND_ADMIN);
          ...
          if (r.brandId && (r.brandId !== brand.id)) {
            errors.push(brandSaveError.ADMIN_ALREADY_EXISTS);
            errorDescription = `${r.email} is already an admin of`;
            const brandMessage = r.brandName ? `(${r.brandName})` : '';
        ...
        errors.push(brandSaveError.INVALID_ADMIN);
      }
    });
  ...
  async getActiveAndInactiveBrandsByCountryId(countryId) {
    const query = super.getAll().orderBy('name');
    query
      .where('country_id', countryId)
