...
const {
  ...
  orderCouponTypes, paymentStatusOrderType,
} = require('../root/enums');
...
class Coupon extends BaseModel {
  ...
  getFiltered(query, countryId, filters) {
    ...
    if (filters?.orderType) {
      query.andWhere('order_type', filters?.orderType);
    }
  ...
  getAll(paging, isValid, searchTerm, countryId, filters) {
    let query = this.db(this.tableName).select('coupons.*');
    let couponDetailType = '';
    if (filters?.type) {
      couponDetailType = filters?.type;
      delete filters?.type;
    }
    if (searchTerm) {
    ...
    query.andWhere('country_id', countryId);
    if (couponDetailType) {
      query.leftJoin('coupon_details', 'coupon_details.coupon_id', `${this.tableName}.id`);
      query.andWhere('coupon_details.type', couponDetailType);
    }
  ...
  async isCouponAvailableForBrandLocation(brandLocationId, couponId) {
    const brandLocationCoupons = await this.roDb('brand_locations_coupons')
      .select('brand_location_id')
      .where('coupon_id', couponId);
    // if there is no brand location was added in the list
    // that mean is all branches selected
    // with UI changes and some flags, we can update this confusing logic
    if (brandLocationCoupons.length === 0) return true;
    return brandLocationCoupons.includes(brandLocationId);
  }
  async isCouponValidForBrandLocation(brandLocationId, couponId) {
    const brandLocationCoupons = await this.roDb('brand_locations_coupons')
      .select('brand_location_id')
      .where('coupon_id', couponId);
    return ((brandLocationCoupons.findIndex(x => x.brandLocationId === brandLocationId)) >= 0) ? true : false;
  }
  ...
  async isValidForThisCustomer(couponCode, customerId, countryId, brandLocationId, orderType = orderCouponTypes.REGULAR_ORDER) {
  ...
  getCouponsAvailableQuery(brandLocationId) {
    brandLocationId = brandLocationId || undefined;
    /*
    ...
    - if brandlocationId is Provided then coupon associated with brandLocation will be returned
    */
    return (
      this.getCouponsQuery() +
      `where
  ...
  async validateCouponSave(coupon) {
    ...
    const couponOld = await this.db(this.tableName)
      .select('id')
      .where('code', code)
      .first();

    if (couponOld && (!id || couponOld.id !== id)) {
      errors.push(couponPayloadError.DUPLICATE_CODE);
    }
  ...
  async save(coupon) {
    ...
    const brandLocationsToSearch = clone(brandLocations);
    const locationRelatedToBrands = [];
    brands.map(async (brandId) => {
      if (brandLocationsToSearch.length > 0) {
        const locationsIds = await this.db('brand_locations')
          .select('id')
          .where('brand_id', brandId)
          .whereIn('id', brandLocationsToSearch);
        if (locationsIds.length > 0) {
          locationsIds.map(brandLocation => {
            locationRelatedToBrands.push({
    ...
    await this.db('brand_locations_coupons')
      .where('coupon_id', couponId)
      .delete();
    if (locationRelatedToBrands.length > 0)
      await this.db('brand_locations_coupons').insert(locationRelatedToBrands);
  ...
  async redeem(code, customerId) {
    ...
    const isValid = await this.isValidForThisCustomer(code, customerId, null, null);
    if (!isValid) {
      ...
      publishEvent(Topic.ANALYTICS_EVENTS,
        {
          ...
          referenceOrderId: transactionId,
          customerId,
        },
        this.context
  ...
  async checkOrderComponents(order, coupon) {
    ...
    try {
      ...
      couponUsable = await this.context.orderSet.couponUsableForOrder(inputWithCustomerId, coupon);
      if (couponUsable) {
        ...
        await this.context.kinesisLogger.sendLogEvent(
          {
            ...
            coupon,
            customerId,
            errorReason: 'coupon is not useable',
          },
          redeemCouponError
        );
  ...
  async isCouponCodeValid(code) {
    const coupon = await this.roDb(this.tableName)
