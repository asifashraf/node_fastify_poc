...
const BranchDistanceMicroserviceManager = require('../branch-distance-microservice/branch-distance-microservice');
const {
  brandLocationError,
  brandLocationStatus,
...
const { calculateStoreStatusFulfillmentKey } = require('./redis-helper');
const { orderTypes } = require('../root/enums');
const { saveCachedOpenings, saveBrandLocationStoreAvailability, invalidateBrandLocationStoreAvailability } = require('./redis-helper');
const { calculateOpeningsKey } = require('./redis-helper');
const {
  saveCachedLocationsInRadius,
  saveCachedLocationsInRadiusUntilNext10MinSection
} = require('./redis-helper');
const { getCachedLocationsInRadius } = require('./redis-helper');
const { calculateInRadiusKey } = require('./redis-helper');
...
const { getCachedBranchesForHomePage, saveCachedBranchesForHomePage, calculateBranchesForHomePage } = require('./redis-helper');
const { tagType } = require('../tag/enums');
...
class BrandLocation extends BaseModel {
  ...
  filterBrandLocations(query, filters) {
    ...
    if (filters.brandId && (filters.brandIds == null || filters.brandIds == undefined)) {
      query.where('brand_locations.brand_id', filters.brandId);
    }
    if ('brandIds' in filters) {
      const uniqBrandIds = [...new Set([...filters.brandIds, filters.brandId].filter(t => !!t))];
      query.whereIn('brand_locations.brand_id', uniqBrandIds);
    }
    if (filters.tagId) {
  ...
  async getWalkingAndDrivingInfo({ location, branchList }) {
    const query = this.db(this.viewBranchBrand)
      .select(
        this.db.raw(`id,
          ST_X(geolocation) as longitude,
          ST_Y(geolocation) as latitude`)
      )
      .whereIn('id', branchList);
    const items = await query;
    return await BranchDistanceMicroserviceManager.getBranchesDistanceForLocation(location, items);
  }
    ...
    if (filters) {
      ...
      if (filterSetId) {
        ...
        if (filterSet) {
          ...
          const { tagIds, brandIds, fulfillmentTypes, offer, freeDelivery, rating } = filterSet;
  ...
  brandLocationsAroundMePayloadMetaData(filterSet, total) {
    const commonPart = {
      ...
      subtitle: {
        en: [total, filterSet?.title?.en || filterSet?.title, 'Shops'].filter(n => n).join(' ').trim().toUpperCase(),
        ar: `\u202B${[total, filterSet?.title?.ar || filterSet?.title, 'متجر'].filter(n => n).join(' ').trim().toUpperCase()}\u202C`,
        tr: [total, filterSet?.title?.tr || filterSet?.title, 'Şubeleri'].filter(n => n).join(' ').trim().toUpperCase(),
      },
    };
    if (filterSet) {
      const {
  ...
  async getBranches(
    ...
    if (filters.isTagged) {
      query.whereIn('id', this.context.tagRelation.getByRelType(tagRelationType.BRAND_LOCATION).select('rel_id'));
    }
    ...
    if (branchListFromMicroservice.length === 0) {
      this.checkEmptyListConditionForBranches(
        query, additionalParameters, fulfillmentType);
    }
    ...
    for (const index in items) {
      ...
      item.storeStatusSortIndex = (item.storeStatus === brandLocationStoreStatus.STORE_CLOSED) ? 1 : 0;
    }
  ...
  async getBranchesNew(
    ...
    if (filters.offer) {
      query.whereIn('id', this.context.tagRelation.getByTagTypes(tagType.OFFER).select('rel_id'));
    }

    if (filters.freeDelivery) {
      if (filters.fulfillmentType == orderFulfillmentTypes.DELIVERY) {
        query.where('delivery_fee', 0);
      } else if (filters.fulfillmentType == orderFulfillmentTypes.EXPRESS_DELIVERY) {
        query.where('express_delivery_fee', 0);
      }
    }

    if (filters.rating) {
      query.whereRaw(`(total_score/total_reviews) in(${filters.rating})`);
    }
    ...
    if (fulfillmentQuery) query.andWhere(fulfillmentQuery, true);
    if (filters.brandId) query.andWhere('brand_id', filters.brandId);
    if (filters.branchRating) query.andWhere(function () {
      this.where('rating', '>=', filters.branchRating);
    });
    const { limit = 5000, offset = 0 } = paging || {};
    query.limit(limit).offset(offset);
  ...
  async validate(brandLocation) {
    ...
    forEach(result.flat(), r => {
      if (r) {
        if ((r.brandId && r.brandLocationId) && (r.brandLocationId !== brandLocation.id)) {
          errors.push(brandLocationError.ADMIN_ALREADY_EXISTS);
          errorDescription = `${r.email} is already an admin of`;
          const brandMessage = r.brandName ? `(${r.brandName})` : '';
          const brandLocationMessage = r.brandLocationName
            ? `(${r.brandLocationName})`
            : '';
          errorDescription += ` branch ${brandLocationMessage} for brand ${brandMessage}`;
        } else if (r.brandId && !r.brandLocationId) {
          errors.push(brandLocationError.BRAND_ADMIN_CAN_NOT_BE_BRANCH_ADMIN);
          errorDescription = `${r.email} is already an admin of`;
          const brandMessage = r.brandName ? `(${r.brandName})` : '';
          errorDescription += ` brand ${brandMessage}`;
        }
      } else {
        errors.push(brandLocationError.INVALID_ADMIN);
      }
    });
            ...
            item.optionSets = await Promise.all(
              optionSets.map(async optionSet => {
                ...
                optionSet.options = options.map(option => {
                  ...
                  return addLocalizationField(option, 'value');
                });
                return addLocalizationField(optionSet, 'label');
              })
            );
            item.tags = await this.context.tagRelation
              .loaders
              .getTagsByRelId
              .load(item.id);
  ...
  async ngLocationsInRadiusLite({
    ...
    if (branchListFromMicroservice.length === 0) {
      this.checkEmptyListConditionForBranches(
        query, { location, radius }, fulfillmentType);
    }
    const branchIds = branchListFromMicroservice.map(function (item) {
      return item.branchId;
    });
    ...
    if (brandLocationList.length != 0) {
      await saveCachedLocationsInRadiusUntilNext10MinSection(targetCacheKey, result);
    }
    return result;
  }
  ...
  checkEmptyListConditionForBranches(query, additionalParameters, fulfillment) {
    const queryClone = query.clone();
    const { maxLocation, minLocation, location, radius } = additionalParameters;
    if (radius && location) {
      let radiusToUse = radius;
      let withinRadiusQuery = `ST_DWithin(
        ST_Transform(geolocation, 7094),
        ST_Transform(ST_SetSRID(ST_MakePoint(?, ?), 4326), 7094),
        ?
        )`;
      ...
      queryClone.andWhereRaw(withinRadiusQuery,
        [
          location.longitude, location.latitude,
          radiusToUse
        ]);
    } else if (location && maxLocation && minLocation) {
      const withinBoxQuery = `
        ST_Intersects(
          ST_MakeEnvelope(?, ?, ?, ?, 4326),
          ST_MakePoint(?, ?))
          `;

      queryClone.andWhereRaw(withinBoxQuery,
        [
          minLocation.longitude, minLocation.latitude,
          maxLocation.longitude, maxLocation.latitude,
          location.longitude, location.latitude
        ]);
    } else {
      return;
    }
    console.log(queryClone.toSQL().toNative());
    console.log(additionalParameters);
    queryClone.then(result => {
      if (result.length > 0) {
        SlackWebHookManager.sendTextAndObjectToSlack(
          'BranchLocations Service returned 0 results',
          JSON.stringify(additionalParameters)
        );
      } else {
        SlackWebHookManager.sendTextToSlack(
          'BranchLocations Service returned 0 results and it is ok');
      }
    });
  }
  ...
  async getAvailableFulfilments(brandLocationId, location) {
    return this.getAllFulfillments(brandLocationId, true, location);
  }
  ...
  async getBrandLocationEndOfDayReport(brandLocationId) {
    /**
     * first check the brand location
     * get weekly schedules
     * if weekly schedules array is empty ( return error ?)
     * find today schedlues
     * if today has schedule
     * then pick the opening and close time of this
     * if current time past to close time get report today
     * if not find last operating day
          recusive
               today sunday we look at previouse
               case 1 => there is some day friday
               case 2 => there is only sunday
     */
    ...
    function getLastOperatingDay(weeklySchedules, currentWeekDay) {
      const schedule = weeklySchedules.find(schedule => schedule.day === currentWeekDay);
      if (schedule) {
        return schedule;
      }
      const previousDay = currentWeekDay - 1 == 0 ? 7 : currentWeekDay - 1;
      return getLastOperatingDay(weeklySchedules, previousDay);
    }

    const brandLocation = await this.context.brandLocation.getById(brandLocationId);
    if (!brandLocation) {
      errors.push(brandLocationReportError.INVALID_BRAND_LOCATION);
      return formatError(errors);
    }

    const weeklySchedules = await this.context.weeklySchedule.getByBrandLocation(brandLocationId);
    if (weeklySchedules.length == 0) {
      errors.push(brandLocationReportError.NO_WEEKLY_SCHEDULES_FOUND_FOR_BRANCH);
      return formatError(errors);
    }

    const weekDay = now.day() + 1;
    const todaySchedule = find(weeklySchedules, schedule => schedule.day === weekDay);
    let isBeforeStoreCloseTime = false;
    if (todaySchedule) {
      // it is closed or not
      const openTime = moment(todaySchedule.openAllDay ? '00:00:00' : todaySchedule.openTime, format).tz(brandLocation.timeZoneIdentifier, true);
      const closeTime = openTime.clone().add(todaySchedule.openAllDay ? 1440 : todaySchedule.openDuration, 'm');
      isBeforeStoreCloseTime = now.isAfter(closeTime);
    }

    if (isBeforeStoreCloseTime) {
      // create report for today
      startTime = moment(todaySchedule.openAllDay ? '00:00:00' : todaySchedule.openTime, format).tz(brandLocation.timeZoneIdentifier, true);
      endTime = startTime.clone().add(todaySchedule.openAllDay ? 1440 : todaySchedule.openDuration, 'm');
    } else {
      const previousWeekDay = weekDay - 1 == 0 ? 7 : weekDay - 1;
      const lastOperatingSchedule = getLastOperatingDay(weeklySchedules, previousWeekDay, (weekDay + 1) % 7);
      let lastOperatingDay = weekDay - lastOperatingSchedule.day;
      lastOperatingDay = lastOperatingDay < 1 ? lastOperatingDay + 7 : lastOperatingDay;
      startTime = moment(lastOperatingSchedule.openAllDay ? '00:00:00' : lastOperatingSchedule.openTime, format).tz(brandLocation.timeZoneIdentifier, true);
      startTime = moment(startTime).subtract(lastOperatingDay, 'd');
      endTime = startTime.clone().add(lastOperatingSchedule.openAllDay ? 1440 : lastOperatingSchedule.openDuration, 'm');
    }

    const todayStart = startTime;
    const todayEnd = endTime;

    let report = {};
    const numberOfTotalOrderquery = await this.roDb({
      bl: this.tableName
    })
      .select(
        this.db.raw(`
          bl.name,b.name_ar,bl.name_tr,bl.id,os.payment_provider,os.current_status,off.type, 
          os.created_at,os.total,os.subtotal,os.fee,os.cash_on_delivery
        `)
      )
      .join({ b: this.context.brand.tableName }, 'bl.brand_id', 'b.id')
      .join({ blwsl: 'brand_location_weekly_schedules' }, 'blwsl.brand_location_id', 'bl.id')
      .join({ os: 'order_sets' }, 'os.brand_location_id', 'bl.id')
      .join({ off: 'order_fulfillment' }, 'os.id', 'off.order_set_id')
      .join({ oss: 'order_set_statuses' }, 'oss.order_set_id', 'os.id')
      .where('bl.id', brandLocationId)
      .whereIn('os.current_status', ['PAYMENT_CANCELED', 'COMPLETED', 'REJECTED'])
      .andWhereBetween('os.created_at', [todayStart, todayEnd]);

    const orderByStatus = groupBy(numberOfTotalOrderquery, 'currentStatus');
    const ordersByFulfilment = groupBy(numberOfTotalOrderquery, 'type');
    const ordersByPaymentMethods = groupBy(numberOfTotalOrderquery, 'paymentProvider');
    const totalInternalSales = ordersByPaymentMethods?.INTERNAL?.length ? [...ordersByPaymentMethods?.INTERNAL.filter(item => item.currentStatus == 'COMPLETED')] : [];
    const totalOnlineSales = ordersByPaymentMethods?.CHECKOUT?.length ? [...ordersByPaymentMethods?.CHECKOUT.filter(item => item.currentStatus == 'COMPLETED')] : [];

    // Peak hours Order Details
    let peakOrdersHoursQuery = this.roDb.raw(`
      SELECT order_sets.created_at,order_sets.id,order_sets.total FROM order_sets
      join brand_locations bl on bl.id=order_sets.brand_location_id
      where  EXTRACT(HOUR FROM order_sets.created_at) in (
      ...
      and bl.id=?
      and order_sets.created_at between ? AND  ?
      GROUP BY order_sets.id,order_sets.created_at`, [todayStart, todayEnd, brandLocationId, todayStart, todayEnd]);
    peakOrdersHoursQuery = await peakOrdersHoursQuery.then(result => transformToCamelCase(result.rows));
    const ordersByPeakHour = {};
    peakOrdersHoursQuery.map(record => {
      const hour = moment.utc(record.createdAt).tz(brandLocation.timeZoneIdentifier).format('hh,A');
      if (!ordersByPeakHour[hour]) {
        ordersByPeakHour[hour] = [];
      }
      record.createdAt = moment.utc(record.createdAt).tz(brandLocation.timeZoneIdentifier).format('DD-MM-YYYY,hh:mm A');
      ordersByPeakHour[hour].push(record);
    });

    const topHours = Object.keys(ordersByPeakHour).sort((a, b) => b - a).slice(0, 5);
    const topHoursWithOrderDetails = [];
    for (let hour of topHours) {
      if (Number(hour.split(',')[0]) > 12) {
        hour = `${(Number(hour.split(',')[0] - 12))},${hour.split(',')[1]}`;
      } else if (Number(hour.split(',')[0]) == 12) {
        hour = `12,${hour.split(',')[1]}`;
      }
      const object = {
        startHour: `${hour.split(',')[0]}:00 ${hour.split(',')[1]}`,
        endHour: hour.startsWith('0') ? `0${(Number(hour.split(',')[0]) + 1)}:00 ${hour.split(',')[1]}` : `${(Number(hour.split(',')[0]) + 1)}:00 ${hour.split(',')[1]}`,
        orders: hour.split(',')[0] == '00' ? ordersByPeakHour[`12,${hour.split(',')[1]}`] : ordersByPeakHour[hour],
        totalSales: ordersByPeakHour[hour]?.reduce(function (acc, obj) {
          return acc + Number(obj.total);
        }, 0)
      };

      topHoursWithOrderDetails.push(object);
    }

    let branchCurrency = this.roDb.raw(`
      SELECT 
        C.SYMBOL,
        C.SYMBOL_AR,
        C.DECIMAL_PLACE,
      ...
      JOIN CURRENCIES C ON BL.CURRENCY_ID = C.ID
      WHERE BL.ID = ?`,
    [brandLocationId]);
    branchCurrency = await branchCurrency.then(result => transformToCamelCase(result.rows));

    /// Top 5 Selling Products
    const topFiveSellingProducts = await this.roDb({
      oi: 'order_items'
    })
      .select(
        this.db.raw('mi.name,mi.name_ar,SUM(oi.quantity) as total_quantity_sold,SUM(os.total) total_sales'))
      .join({ os: 'order_sets' }, 'os.id', 'oi.order_set_id')
      .join({ mi: 'menu_items' }, 'mi.id', 'oi.menu_item_id')
      .join({ bl: 'brand_locations' }, 'bl.id', 'os.brand_location_id')
      .where('bl.id', brandLocationId)
      .andWhereBetween('os.created_at', [todayStart, todayEnd])
      .groupBy('mi.id')
    ...
    report = {
      reportDate: startTime.format('DD-MM-YYYY'),
      branchName: brandLocation?.name,
      branchNameAr: brandLocation?.nameAr,
      branchCurrency: branchCurrency[0],
      totalOrders: numberOfTotalOrderquery.length,
      ordersByStatus: {
        completed: orderByStatus?.COMPLETED?.length || 0,
        rejected: orderByStatus?.REJECTED?.length || 0,
        canceled: orderByStatus?.PAYMENT_CANCELED?.length || 0
      },
      ordersByFulfilmentType: {
        // ...(brandLocation.allowDeliverToCar) && { car: ordersByFulfilment?.CAR?.length || 0 },
        // ...(brandLocation.hasDelivery) && { delivery: ordersByFulfilment?.DELIVERY?.length || 0 },
        // ...(brandLocation.hasPickup) && { pickup: (ordersByFulfilment?.PICKUP?.length || 0) },
        // ...(brandLocation.allowExpressDelivery) && { expressDelivery: (ordersByFulfilment?.EXPRESS_DELIVERY?.length || 0) },
        car: (ordersByFulfilment?.CAR?.length || 0),
        delivery: (ordersByFulfilment?.DELIVERY?.length || 0),
        pickup: (ordersByFulfilment?.PICKUP?.length || 0),
        expressDelivery: (ordersByFulfilment?.EXPRESS_DELIVERY?.length || 0),
      },

      ordersByFulfilmentTypePerStatus: {
        //...(brandLocation.hasPickup) && {
        pickup: {
          completed: groupBy(ordersByFulfilment?.PICKUP, 'currentStatus').COMPLETED?.length || 0,
          rejected: groupBy(ordersByFulfilment?.PICKUP, 'currentStatus').REJECTED?.length || 0,
          canceled: groupBy(ordersByFulfilment?.PICKUP, 'currentStatus').PAYMENT_CANCELED?.length || 0
        }, // }
        //...(brandLocation.hasDelivery) && {
        delivery: {
          completed: groupBy(ordersByFulfilment?.DELIVERY, 'currentStatus').COMPLETED?.length || 0,
          rejected: groupBy(ordersByFulfilment?.DELIVERY, 'currentStatus').REJECTED?.length || 0,
          canceled: groupBy(ordersByFulfilment?.DELIVERY, 'currentStatus').PAYMENT_CANCELED?.length || 0
        }, //}
        // ...(brandLocation.allowDeliverToCar) && {
        car: {
          completed: groupBy(ordersByFulfilment?.CAR, 'currentStatus').COMPLETED?.length || 0,
          rejected: groupBy(ordersByFulfilment?.CAR, 'currentStatus').REJECTED?.lengt || 0,
          canceled: groupBy(ordersByFulfilment?.CAR, 'currentStatus').PAYMENT_CANCELED?.length || 0
        }, //}
        // ...(brandLocation.allowExpressDelivery) && {
        expressDelivery: {
          completed: groupBy(ordersByFulfilment?.EXPRESS_DELIVERY, 'currentStatus').COMPLETED?.length || 0,
          rejected: groupBy(ordersByFulfilment?.EXPRESS_DELIVERY, 'currentStatus').REJECTED?.lengt || 0,
          canceled: groupBy(ordersByFulfilment?.EXPRESS_DELIVERY, 'currentStatus').PAYMENT_CANCELED?.length || 0
        }
        //  }
      },
      salesPerFulfilmentType: {
        // ...(brandLocation.allowDeliverToCar) && { car: groupBy(orderByStatus?.COMPLETED, 'type').CAR?.reduce((a, b) => ({ total: Number(a.total) + Number(b.total) }))?.total || 0 },
        // ...(brandLocation.hasDelivery) && { delivery: groupBy(orderByStatus?.COMPLETED, 'type').DELIVERY?.reduce((a, b) => ({ total: Number(a.total) + Number(b.total) }))?.total || 0 },
        // ...(brandLocation.hasPickup) && { pickup: groupBy(orderByStatus?.COMPLETED, 'type').PICKUP?.reduce((a, b) => ({ total: Number(a.total) + Number(b.total) }))?.total || 0 },
        // ...(brandLocation.allowExpressDelivery) && { expressDelivery: groupBy(orderByStatus?.COMPLETED, 'type').EXPRESS_DELIVERY?.reduce((a, b) => ({ total: Number(a.total) + Number(b.total) }))?.total || 0 }
        car: groupBy(orderByStatus?.COMPLETED, 'type').CAR?.reduce((a, b) => ({
          total: Number(a.total) + Number(b.total)
        }))?.total || 0,

        delivery: groupBy(orderByStatus?.COMPLETED, 'type').DELIVERY?.reduce((a, b) => ({
          total: Number(a.total) + Number(b.total)
        }))?.total || 0,

        pickup: groupBy(orderByStatus?.COMPLETED, 'type').PICKUP?.reduce((a, b) => ({
          total: Number(a.total) + Number(b.total)
        }))?.total || 0,

        expressDelivery: groupBy(orderByStatus?.COMPLETED, 'type').EXPRESS_DELIVERY?.reduce((a, b) => ({
          total: Number(a.total) + Number(b.total)
        }))?.total || 0
      },
      ...
      totalSales: {
        inStore: {
          numberOfOrders: ordersByPaymentMethods?.CHECKOUT?.filter(item => item.currentStatus == 'COMPLETED')?.length || 0,
          totalSales: totalOnlineSales.length > 0 ? totalOnlineSales.reduce((a, b) => ({
            total: Number(a.total) + Number(b.total)
          }))?.total || 0 : 0
        },
        online: {
          numberOfOrders: ordersByPaymentMethods?.INTERNAL?.filter(item => item.currentStatus == 'COMPLETED')?.length || 0,
          totalSales: totalInternalSales.length > 0 ? totalInternalSales.reduce((a, b) => ({
            total: Number(a.total) + Number(b.total)
          }))?.total || 0 : 0
        }
      },

      orderByPeakHours: topHoursWithOrderDetails,
      topSellingProductsWithSalesInfo: topFiveSellingProducts
    };

    report.grandTotalSales = (totalOnlineSales.length > 0 ? totalOnlineSales.reduce((a, b) => ({
      total: Number(a.total) + Number(b.total)
    }))?.total || 0 : 0) +
      (totalInternalSales.length > 0 ? totalInternalSales.reduce((a, b) => ({
        total: Number(a.total) + Number(b.total)
      }))?.total || 0 : 0);
    return {report};
  }
  ...
  getBranchesInRadius(
    ...
    if (fulfillment === fulfillmentType.DELIVERY) {
      ...
      query.andWhere(
        this.roDb.raw(`
          ST_Contains(bla.express_delivery_zone, ST_SetSRID(ST_MakePoint(:longitude, :latitude), 4326))`, { longitude, latitude }),
      );
    } else {
      query.andWhere(
        this.roDb.raw(`
  ...
  getBranchesOfBrand(latitude, longitude, brandIds) {
    const query = this.getBranchListWithDistanceByLocation(latitude, longitude);
    if (!Array.isArray(brandIds)) brandIds = [brandIds];
    query.whereIn('b.id', brandIds);
    return query;
  }

  getBranchesByIds(latitude, longitude, brandLocationIds) {
    const query = this.getBranchListWithDistanceByLocation(latitude, longitude);
    if (!Array.isArray(brandLocationIds)) brandLocationIds = [brandLocationIds];
    query.whereIn('bl.id', brandLocationIds);
    query.toString();
    return query;
  }
  ...
  async getAllFulfillments(brandLocationId, onlyAvailable = false, location) {
    ...
    if (
      ...
      const customerAddressesStatus = await this.context.customerAddress.isCustomerInDeliverySphere(brandLocationId, location);
      availableFulfillment.customerAddressesStatus = customerAddressesStatus;
      availableFulfillments.push(availableFulfillment);
      /*
      if (location) {
        const deliveryInclude = await this.context.brandLocationAddress.isCustomerInDeliverySphere(brandLocationId, location);
        if (deliveryInclude) {
          availableFulfillments.push({
            countryCnfKey: countryConfigurationKeys.ESTIMATED_TIME_IN_MINS_DELIVERY,
            fulfillment: orderTypes.DELIVERY,
    ...
    if (
      ...
      const customerAddressesStatus = await this.context.customerAddress.isCustomerInExpressDeliveryZone(brandLocationId, location);
      availableFulfillment.customerAddressesStatus = customerAddressesStatus;
      availableFulfillments.push(availableFulfillment);
      /*
      if (location) {
        const expressDeliveryInclude = await this.context.brandLocationAddress.isCustomerInExpressDeliveryZone(brandLocationId, location);
        if (expressDeliveryInclude) {
          availableFulfillments.push({
            countryCnfKey: countryConfigurationKeys.ESTIMATED_TIME_IN_MINS_EXPRESS_DELIVERY,
            fulfillment: orderTypes.EXPRESS_DELIVERY,
            minimumOrderAmount: response.minimumExpressDeliveryOrderAmount,
  ...
  async getExcludedBrandLocationsByTagId(id) {
    const query = `
    SELECT * FROM brand_locations AS bl 
    WHERE bl.brand_id IN
    (
      SELECT bl.brand_id FROM tag_relations tr 
    ...
    AND bl.id NOT IN
    (
      SELECT rel_id FROM tag_relations tr2 
      WHERE tr2.rel_type = ?
      AND tr2.tag_id = ?
    )
    AND bl.status = 'ACTIVE'`;
    const results = await this.db
      .raw(query, [tagRelationType.BRAND_LOCATION, id, tagRelationType.BRAND_LOCATION, id])
      .then(result => transformToCamelCase(result.rows));
    const excludecdGroupedBrandLocations = groupBy(results, 'brandId');
    ///selected brandLocations
    const selectedBrandLocationsQuery = `SELECT * FROM brand_locations AS bl 
      where bl.id in (
      select rel_id from tag_relations tr where tr.rel_type = ? and 
      tr.tag_id = ?) AND bl.status = 'ACTIVE'`;
    const selectedBrandLocations = await this.db
      .raw(selectedBrandLocationsQuery, [tagRelationType.BRAND_LOCATION, id])
      .then(result => transformToCamelCase(result.rows));
    const selectedGroupedBrandLocations = groupBy(selectedBrandLocations, 'brandId');
    const countryIdQuery = `select c.id  from countries c
    join brand_locations bl on c.currency_id=bl.currency_id
    join tag_relations tr on tr.rel_id = bl.id
    where tr.tag_id = ? limit 1`;
    const countryId = await this.db
      .raw(countryIdQuery, [id])
      .then(result => transformToCamelCase(result.rows));

    //prepare response Object
    const responseArray = [];
    const branchesToMap = excludecdGroupedBrandLocations.length > 0 ? excludecdGroupedBrandLocations : selectedGroupedBrandLocations;
    for await (const brandId of Object.keys(branchesToMap)) {
      const allbrandLocations = await this.getByBrandId(brandId);
      const allSelectedBrandLocationIds = selectedGroupedBrandLocations[brandId].map(i => i.id);
      responseArray.push({
        countryId: countryId.length > 0 ? countryId[0]?.id : null,
        brandId,
        selectedBrandLocations: addLocalizationMultipleFields(selectedGroupedBrandLocations[brandId], ['name', 'brandDescription']),
        excludedBrandLocations: excludecdGroupedBrandLocations[brandId] ? addLocalizationMultipleFields(excludecdGroupedBrandLocations[brandId], ['name', 'brandDescription']) : [],
        tagAssignmentType: allbrandLocations.map(i => i.id).every(i => allSelectedBrandLocationIds.includes(i)) ? 'BRAND' : 'BRANCH',
        excludeBranchesCheck: results.length > 0 ? true : false
      });
    }
  ...
  async prepareDataforBranchLocationswithTimeZones(brandLocations, brands, excludeBranches) {
    ...
    if (brandLocations?.length > 0) {
      const locationsIds = await this.db(`${this.tableName}`)
  ...
  async checkLocationInExpressZone(location) {
    const { longitude, latitude } = location;
    const { count: brandLocationNumber} = await this.roDb(`${this.context.brandLocationAddress.tableName} as bla`)
      .count('*')
      .leftJoin(`${this.context.brandLocation.tableName} as bl`, 'bl.id', 'bla.brand_location_id')
      .leftJoin(`${this.context.brand.tableName} as b`, 'b.id', 'bl.brand_id')
      .where('bl.status', brandLocationStatus.ACTIVE)
      .where('b.status', brandStatus.ACTIVE)
      .andWhere(
        this.roDb.raw(`
