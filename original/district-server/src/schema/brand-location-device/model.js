...
const { env, redisTimeParameter } = require('../../../config');
...
const { acceptedOrderSetKey, preparingOrderSetKey, rejectedOrderKey, reportedOrderKey } = require('../../../redis/keys');
...
class BrandLocationDevice extends BaseModel {
  ...
  async isValidDeviceCode({ code, brandLocationId }) {
    ...
    const pairedDevice = await this.getDeviceByBrandLocationId(brandLocationId);
    if (pairedDevice) {
      errors.push(mposDeviceError.BRAND_LOCATION_ALREADY_PAIRED);
    }
  ...
  async acceptOrder(deviceId, orderSetId) {
    ...
    try {
      const acceptedOrderSetDetail = { deviceId, orderSetId };
      const cachedOrderSet = await redis.getByKeyAndParse(`${acceptedOrderSetKey}${orderSetId}`);
      if (cachedOrderSet) {
        return { success: false, message: 'Ambiguous Request' };
      }
      redis.set(
        `${acceptedOrderSetKey}${orderSetId}`,
        JSON.stringify(acceptedOrderSetDetail),
        'EX',
        redisTimeParameter.fiveMinutesInSeconds
      );
  ...
  async preparingOrder(deviceId, orderSetId) {
    ...
    try {
      const preparingOrderSetDetail = { deviceId, orderSetId };
      const cachedOrderSet = await redis.getByKeyAndParse(`${preparingOrderSetKey}${orderSetId}`);
      if (cachedOrderSet) {
        return { success: false, message: 'Ambiguous Request' };
      }
      redis.set(
        `${preparingOrderSetKey}${orderSetId}`,
        JSON.stringify(preparingOrderSetDetail),
        'EX',
        redisTimeParameter.fiveMinutesInSeconds
      );
        ...
        orderSetIds.map(async orderSetId => {
          if (isUUID.v4(orderSetId)) {
            ...
            if (orderSet) {
              ...
              SlackWebHookManager.sendTextToSlack(`[INFO] [${env}] [/order-status-completed] Referral and rewards point process completed for the order (${orderSetId}), isRewardAdded:${isRewardAdded}, isReferralAdded:${isReferralAdded}`, mposWebhookUrl);
  ...
  async rejectedOrder(deviceId, orderSetId, rejectedReason) {
    ...
    try {
      const rejectedOrderSetDetail = { deviceId, orderSetId };
      const cachedOrderSet = await redis.getByKeyAndParse(`${rejectedOrderKey}${orderSetId}`);
      if (cachedOrderSet) {
        return { success: false, message: 'Ambiguous Request' };
      }
      redis.set(
        `${rejectedOrderKey}${orderSetId}`,
        JSON.stringify(rejectedOrderSetDetail),
        'EX',
        redisTimeParameter.fiveMinutesInSeconds
      );
  ...
  async reportedOrder(deviceId, orderSetId, reportedReason) {
    ...
    try {
      const reportedOrderSetDetail = { deviceId, orderSetId };
      const cachedOrderSet = await redis.getByKeyAndParse(`${reportedOrderKey}${orderSetId}`);
      if (cachedOrderSet) {
        return { success: false, message: 'Ambiguous Request' };
      }
      redis.set(
        `${reportedOrderKey}${orderSetId}`,
        JSON.stringify(reportedOrderSetDetail),
        'EX',
        redisTimeParameter.fiveMinutesInSeconds
      );
    ...
    */
    ...
    for (let i = 1; i < 8; i++) {
      ...
      if (schedule) {
        ...
        if (branch.hasPickup) {
          obj = {
            ...obj, 'pickup': schedule.openAllDay
              ? serviceTime :
              (schedule.openTime ?
        ...
        if (branch.hasDelivery) {
          obj = {
            ...obj, 'delivery': schedule.openAllDay
              ? serviceTime :
        ...
        if (branch.allowExpressDelivery) {
          obj = {
            ...obj, 'expressDelivery': schedule.openAllDay
              ? serviceTime :
    ...
    try {
      ...
      if (acceptingOrder) {
        await this.db(this.tableName)
          ...
          .andWhere('status', mposDeviceStatus.PAIRED)
          .update({ offUntil: null });
        await this.context.brandLocation.setBrandLocationAcceptingOrders(device.brandLocationId, acceptingOrder);
      } else {
        if (offlineTime) {
          const now = moment();
          offUntil = now.clone();
          ...
          await this.db(this.tableName)
            .update({ offUntil })
            .where('id', device.id);
          if (offUntil !== null) await this.context.brandLocation.setBrandLocationAcceptingOrders(device.brandLocationId, acceptingOrder,
            isNewAppRequest ? reason : 'First Barista App');
        } else {
          await this.db(this.tableName)
            .where('id', device.id)
            .andWhere('status', 'PAIRED')
            .update({ offUntil: null });
          await this.context.brandLocation.setBrandLocationAcceptingOrders(device.brandLocationId, acceptingOrder, isNewAppRequest ? reason : 'First Barista App');
        }
      }
      ...
      return {
        success: true, message: 'Branch Availability Updated',
    ...
    let orders = await this.roDb('order_set_arriving_times as osat')
      ...
      .andWhereRaw(
        `(os.current_status <> '${orderSetStatusNames.INITIATED}'::order_set_statuses_enum AND
          ...
          os.current_status <> '${orderSetStatusNames.REJECTED}'::order_set_statuses_enum AND
          os.current_status <> '${orderSetStatusNames.COMPLETED}'::order_set_statuses_enum AND
  ...
  async getBrandLocationEndOfDayReport(deviceId) {
    const device = await this.getDeviceByDeviceId(deviceId);

    if (!device) {
      SlackWebHookManager.sendTextToSlack(`[INFO] [${env}] [/branch-end-of-day-report] Unknown device (${deviceId}) requesting data, request has been denied.`, mposWebhookUrl);
      return { success: false, message: 'Invalid Device' };
    }
    const report = await this.context.brandLocation.getBrandLocationEndOfDayReport(device.brandLocationId);
    if (report?.errors?.length > 0) {
      return { success: false, error: report.error, errors: report.errors};
    } else {
