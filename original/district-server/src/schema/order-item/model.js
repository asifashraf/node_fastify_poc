...
class OrderItem extends BaseModel {
  ...
  async getWithMenuItemByOrderSetId(orderSetId) {
    return addLocalizationField(
      await this.loaders.byOrderSetWithMenuItem.load(orderSetId),
      'name'
    );
  }

  async getWithMenuItemOptionsByOrderSetId(orderSetId) {
    return addLocalizationField(
      await this.loaders.byOrderSetWithDetail.load(orderSetId),
      'name'
    );
  }
  ...
  async orderItemsAvailability({ orderSetId, brandLocationId }) {
    ...
    let item;
    const orderItemIds = await Promise.all(map(orderSetItems, async item => { return item.id; }));
    const orderOptions = flatten(await this.context.orderItemOption.loaders.byOrderItem.loadMany(orderItemIds));
    const allOptions = {};
    for (const itemId of orderItemIds) {
      const itemOption = filter(orderOptions, orderOption => orderOption.orderItemId == itemId);
      allOptions[itemId] = itemOption;
    }
    const missingItems = [];
    ...
    const uniqueMissingItemNames = [];
    const uniqueEn = [];
    for (let i = 0; i < missingItemNames.length; i++) {
      if (!uniqueEn.includes(missingItemNames[i].en)) {
        uniqueEn.push(missingItemNames[i].en);
        uniqueMissingItemNames.push(missingItemNames[i]);
      }
    }
    ...
    return { missingItemNames: uniqueMissingItemNames, orderSet, availabilityStatus };
  }
}
