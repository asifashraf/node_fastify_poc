...
const { calculateIconButtonItemsKey, getCachedIconButtonItems, saveCachedIconButtonItems, invalidateIconButtonItems } = require('./redis-helper');
class IconButtonItem extends BaseModel {
  ...
  async getAllItems({ refQueryId, countryId, filters }) {
    const cachedKey = calculateIconButtonItemsKey(countryId);
    const cachedData = await getCachedIconButtonItems(cachedKey);
    if (cachedData && cachedData.length > 0) {
      return cachedData;
    }
    const items = await this.db.table(this.tableName)
      .select(
        this.roDb.raw(
    ...
    if (data && data.length > 0) await saveCachedIconButtonItems(cachedKey, data);
    return data;
  }
  async getIconsAndSettingsByCountryIdAndDeeplink(deeplink, image, countryId) {
    const icons = await this.db(this.tableName)
      .leftJoin(
        'home_page_icon_button_item_settings as hp',
        'hp.item_id',
        'home_page_icon_button_items.id'
      )
      .where('home_page_icon_button_items.deeplink', deeplink)
      .where('home_page_icon_button_items.image', image)
      .where('hp.country_id', countryId)
      .then(transformToCamelCase)
      .then(first);
    return addLocalizationMultipleFields(icons, ['title']);
  }

  async validateIconButtonItem(iconButtonItemInfo, iconButtonItemSettingInfo) {

    const errors = [];
    if (!iconButtonItemInfo.image || !iconButtonItemInfo.title || !iconButtonItemInfo.deeplink) {
  ...
  async saveItemAndSettings({ iconButtonItemInfo, iconButtonItemSettingInfo }) {
    ...
    await invalidateIconButtonItems();
    let itemId;
