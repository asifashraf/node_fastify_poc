...
const { getGiftCardUrl } = require('../../lib/dynamic-url');
...
class GiftCardOrder extends BaseModel {
  ...
  async create(input) {
    ...
    publishEvent(Topic.ANALYTICS_EVENTS,
      {
        eventType: customerAnalyticsEvents.GIFT_CARD_PURCHASE_COUNT,
  ...
  async prepareEInvoice({giftCardOrder}) {
    try {
      const country = await this.context.country.getById(giftCardOrder.countryId);
      const components = [
        {
      ...
      createEInvoice({
        ...
        orderCreatedAt: giftCardOrder.created,
        orderShortCode: giftCardOrder.shortCode,
        customerId: giftCardOrder.customerId,
        countryId: country.id,
        items,
        components
