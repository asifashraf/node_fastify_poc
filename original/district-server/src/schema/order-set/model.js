...
class OrderSet extends BaseModel {
  ...
  async getDeliveryStatusByShortCode(shortCode) {
    ...
    if (shortCode) {
      ...
      //Check If deliveryPartner is TALABAT
          ...
          `, shortCode);
            ...
            if (deliveryOrderStatus) {
              if (estimatedTime !== null) {
                ...
                estimatedTime = isNaN(estimatedTime) ? 0.0 : (estimatedTime < 0 ? 0.0 : estimatedTime);
              }
  ...
  getDeliveryStatusHistoryByShortCode(shortCode) {
    return this.roDb.select(
      'id',
      'delivery_partner',
      'partner_order_id',
      'status',
      'created_at'
    ).from('delivery_order_statuses')
      .where('reference', shortCode)
    ...
    const ordersQuery = this.roDb
      .select(
        this.roDb
          ...
          current_status = '${orderSetStatusNames.DELIVERED}'::order_set_statuses_enum OR
          current_status = '${orderSetStatusNames.PAYMENT_FAILURE}'::order_set_statuses_enum OR
          current_status = '${orderSetStatusNames.PAYMENT_CANCELED}'::order_set_statuses_enum) THEN true ELSE false END AS is_past `)
      )
      .from('order_sets')
...
`;
  ...
  async generateIntIdForOrderSet(orderSetId, shortCode) {
    try {
      const id = await this.db('order_set_int_ids').insert([{
        order_set_id: orderSetId,
        order_set_shortcode: shortCode
      }], ['id']);
  ...
  async isCashBackAdded(orderSetId, customerId, currencyId) {
    const { count } = await this.db('loyalty_transactions')
      .count('*')
      .where('customer_id', customerId)
      .where('reference_order_id', orderSetId)
      .where('currency_id', currencyId)
      .andWhereRaw('credit > 0')
      .first();
  ...
  async addCashback(orderSetId) {
    ...
    if (orderSet && orderSet.isCashbackCoupon && orderSet.couponAmount > 0) {
      if (!await this.isCashBackAdded(orderSetId, orderSet.customerId, orderSet.currencyId)) {
  ...
  async paidWithCash(orderSetId, wasPaid, note) {
    const orderSet = await this.getById(orderSetId);
...
`;
  ...
  async addFilterOrderSetLiteQuery({ query, filters }) {
    ...
    if (searchTerm) {
      ...
      query.andWhere(function () {
        this.orWhereRaw('short_code ilike ?', [`%${searchTerm}%`]);
        this.orWhereRaw('brand_name ilike ?', [`%${searchTerm}%`]);
        this.orWhereRaw('branch_name ilike ?', [`%${searchTerm}%`]);
        this.orWhereRaw('customer_id ilike ?', [`%${searchTerm}%`]);
        this.orWhereRaw('customer_first_name ilike ?', [`%${searchTerm}%`]);
        this.orWhereRaw('customer_last_name ilike ?', [`%${searchTerm}%`]);
        this.orWhereRaw('customer_last_name ilike ?', [`%${searchTerm}%`]);
        this.orWhereRaw('customer_email ilike ?', [`%${searchTerm}%`]);
        this.orWhereRaw('customer_phone_number ilike ?', [`%${searchTerm}%`]);
      });
    }
    if (filterType) {
  ...
  async getOrderDetailWithBrandLocation(brandLocationId, orderSetId) {
    let order = await this.roDb
      .select(
        this.roDb.raw(
          ...
          of.vehicle_color, of.vehicle_plate_number, of.deliver_to_vehicle, osat.arrival_time, osat.arrived as customer_arrived, dos.delivery_partner`
        )
      )
      .from('order_sets as os')
      .join('customers as c', 'c.id', 'os.customer_id')
      .join('order_fulfillment as of', 'of.order_set_id', 'os.id')
      .leftJoin('delivery_orders as dos', 'dos.order_id', 'os.id')
    ...
    const {countryId} = await this.context.brand.getById(order.brandId);
    const fulfilment = pick(order, omitListFulfillment);
    order.fulfillment = fulfilment;
    const dbConfig = await this.context.countryConfiguration.getByKey(
      countryConfigurationKeys['ESTIMATED_TIME_IN_MINS_' + fulfilment.fulfillmentType],
      countryId
    );
    if (dbConfig && dbConfig.configurationValue) {
      order.fulfillment.estimatedTimeInMins = dbConfig.configurationValue;
      order.fulfillment.estimatedTime = moment(order.createdAt).add(dbConfig.configurationValue, 'minutes');
    }
