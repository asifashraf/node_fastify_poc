...
const renderTemplate = (orderSet, successful, knetResponse) => {
  ...
  const isExpressDelivery = orderSet.fulfillment.type === 'EXPRESS_DELIVERY';
  const deliveryFulfillmentName = isDelivery ? 'Delivery' : isExpressDelivery ? 'Express Delivery' : null;
  ...
  if (deliveryAddress && Array.isArray(deliveryAddress.extraFields)) {
    const extraFields = deliveryAddress.extraFields.filter(field => field.value);
    if (extraFields.length > 0) {
      customerAddress = extraFields.map(field => `${field.name[preferredLanguage]}: ${field.value}`).join(', ');
    }
  }
