...
const { applyHooks } = require('../database/hooks');
...
const Kiosk = require('./schema/kiosk/model');
...
const TagSchedules = require('./schema/tag-schedules/model');
const PaymentMethod = require('./schema/payment-method/model');
...
const HorizontalCardListItem = require('./schema/home-page-horizontal-card-list-item/model');
...
const RewardTierPerkMenuItem = require('./schema/reward-tier-perk-menu-item/model');
const InvalidCouponAttemps = require('./schema/invalid-coupon-attempts/model');
const AdminLoginService = require('./schema/admin/login/login');
const FilterSet = require('./schema/filter-set/model');
const TapCustomer = require('./schema/tap-customer/model');
const AbandonedCart = require('./schema/abandoned-cart/model');
...
class QueryContext {
  ...
  constructor(dbHandles, redis, auth, pubsub, schema, req = {}) {
    ...
    this.kinesisLogger = {
      sendLogEvent: async (eventObject, eventType, indexName, logType = 'Default') => {
        try {
          ...
          if (logType == 'Admin') {
            await kinesisLogger.sendAdminLogEvent(msgObj, eventType);
          } else {
...
RequestId: ${this.requestId}`);
  ...
  get menuIntegrationServices() {
    if (this._menuItegrationServices === undefined) {
      this._menuItegrationServices = MenuIntegrationServices(this);
    }
    return this._menuItegrationServices;
  }
  ...
  get kiosk() {
    if (this._kiosk === undefined) {
      this._kiosk = new Kiosk(
        this.db,
        this
      );
    }
  ...
  get tagSchedule() {
    if (this._tagSchedule === undefined) {
      this._tagSchedule = new TagSchedules(
        this.db,
        this
      );
    }
    return this._tagSchedule;
  }
  ...
  get horizontalCardListItem() {
    if (this._horizontalCardListItem === undefined) {
      this._horizontalCardListItem = new HorizontalCardListItem(
        this.db,
        this,
      );
    }
    return this._horizontalCardListItem;
  }
  ...
  get adminLogin() {
    if (this._adminLogin === undefined) {
      this._adminLogin = new AdminLoginService(
        this.db,
        this
      );
    }
  ...
  get invalidCouponAttemps() {
    if (this._invalidCouponAttemps === undefined) {
      this._invalidCouponAttemps = new InvalidCouponAttemps(
        this.db,
        this
      );
    }
    return this._invalidCouponAttemps;
  }

  get abandonedCart() {
    if (this._abandonedCart === undefined) {
      this._abandonedCart = new AbandonedCart(
        this.db,
        this
      );
    }
    return this._abandonedCart;
  }
  ...
  get rewardTierPerkMenuItem() {
    if (this._rewardTierPerkMenuItem === undefined) {
      this._rewardTierPerkMenuItem = new RewardTierPerkMenuItem(
        this.db,
        this
      );
    }
    return this._rewardTierPerkMenuItem;
  }
