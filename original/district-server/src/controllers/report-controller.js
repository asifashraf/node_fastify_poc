...
const { getAuthUser } = require('../lib/util');
...
exports.exportCreditOrdersReport = async (req, res) => {
  ...
  if (admin && !auth.isVendorAdmin && auth.permissions.includes('creditOrders:export')) {
    const { sku, startDate, endDate, countryId } = req.query;
    return context.loyaltyOrder.getAllToCSV(
      csvStream,
      sku,
      startDate,
      endDate,
  ...
  return res.status(401).send('Insufficient authority');
};
...
exports.exportOrdersReport = async (req, res) => {
  ...
  if (hasPermission) {
    ...
    adminActivityLog.query = req.query;
    await context.kinesisLogger.sendLogEvent(
      adminActivityLog,
      kinesisAdminEventTypes.ordersReportDownloadAdminEvent,
      'Admin'
    );
