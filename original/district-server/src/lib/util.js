...
function displayServer(server) {
  if (!process?.argv?.includes('localDev')) {
    SlackWebHookManager.sendTextToSlack(`[APP_STARTED][${os.hostname()}]`).then(r => console.log(r));
  }
...
const getS3BucketAndKeyFromUrl = (inputString) => {
  try {
    const u = new URL(inputString);
    if (!u.host.includes('s3') || !u.host.includes('amazonaws')) return inputString;
    const regex = /^\/([^/]+)\/(.*)$/;
    return u.pathname.match(regex);
  } catch (error) {
    return false;
  }
