...
async function initiateGatewayRequest(currencyCode, countryCode) {
  ...
  if (!mfConfig.gatewayUrl) {
    return {
      error: `MyFatoorah GatewayUrl config for country ${countryCode} not found`,
    };
  }
...
async function initiatePayment(
  ...
  return axios
    ...
    .then(async response => {
      const { data } = response;
      if (data.IsSuccess) {
        const providerPaymentMethods = await context.paymentMethod.getProviderPaymentMethodsByCountryCode(paymentProvider.MY_FATOORAH, countryCode);
        ...
        data.Data.PaymentMethods.map(pm => {
          ...
          if (
            (pm.PaymentMethodId === 1 && providerPaymentMethods?.isKnetEnable) || // include KNET only for KW
            (pm.PaymentMethodId === 6 && providerPaymentMethods?.isMadaEnable) || // include MADA only for SA
            // (pm.PaymentMethodId === 6 && providerPaymentMethods?.isMadaEnable) || // include Debit Cards UAE only for AE
            ...
            (
              client.os === 'ios' && pm.PaymentMethodId === 11
              && providerPaymentMethods?.isApplePayEnable
              && checkMinRequiredClientVersion(context.req.headers, '6.8.4.1')
            )
          ) {
            firstPaymentMethods.push(paymentMethod);
          } else {
            let isEnable = false;
            switch (pm.PaymentMethodId) {
              case 2:
                isEnable = providerPaymentMethods?.isVisaMasterEnable;
                break;
              case 3:
                isEnable = providerPaymentMethods?.isAmexEnable;
                break;
              case 12:
                isEnable = providerPaymentMethods?.isStcEnable;
                break;
              default:
                break;
            }
            if (isEnable) paymentMethods.push(paymentMethod);
          }
          /*
          if (
            (
...
async function executePayment(
  ...
  const { currency, country: {timeZoneIdentifier}, mfConfig, error } = await initiateGatewayRequest.apply(
    { db },
    [currencyCode, countryCode]
  );
  if (error) {
  ...
  const requestData = {
    ...
    ExpiryDate: moment().tz(timeZoneIdentifier).add(mfConfig.offsetExpiryDateInMinutes, 'minutes').format()
  };
  if (paymentMethod === '1') {
    /**
