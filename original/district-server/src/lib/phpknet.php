...
/* This script is used to parse a KNET resource file.
It was found from https://github.com/GDGKuwait/knet-php-demo/blob/master/com/aciworldwide/commerce/gateway/plugins/e24PaymentPipe.inc.php
To use it:
- install php (with ZipArchive support) 
- place the knet provided resource.cgn file in the same dir as this script
- run it with `php phpknet.php` and review the logs
...
class e24PaymentPipe {
    ...
    function sendMessage($s, $s1) {
        ...
        if ($this->port == "443") {
            ...
            $stringbuffer.=($s1);
            $this->debugMsg.=("<br>About to create the URL to: " . $stringbuffer . " with data " . $s . " ");
            $url = $stringbuffer;
...
$Pipe->setResponseURL("responseurl.php"); // set your respone page URL
$Pipe->setErrorURL("errorurl.php"); //set your error page URL
$Pipe->setAmt("10"); //set the amount for the transaction
//$Pipe->setResourcePath("/Applications/MAMP/htdocs/php-toolkit/resource/");
$Pipe->setResourcePath("./"); //change the path where your resource file is
$Pipe->setAlias("cofe");
$Pipe->getSecureSettings();
