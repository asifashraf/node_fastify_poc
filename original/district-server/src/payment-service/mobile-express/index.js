...
class MobileExpress {
  ...
  getCofePaymentError(response) {
    const { paymentResult, paymentList, resultMessage } = response;
    ...
    if (
      ...
      && paymentList[0].resultCategory
    ) {
      return {
        ...findErrorCode(paymentList[0].resultCategory.categoryCode),
        providerDescription: paymentList[0].resultCategory.categoryName
      };
    }
    return {
      ...findErrorCode(paymentResult),
      providerDescription: resultMessage
    };
  }
}
