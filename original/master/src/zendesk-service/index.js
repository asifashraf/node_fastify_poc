...
class ZendeskService {
  constructor(db, context) {
    ...
    this.baseTicket = {
      subject: zendeskServiceConfig.subject,
      comment: zendeskServiceConfig.comment,
    };
  }
  ...
  async createZendeskTicket(orderId, requesterEmail) {
    ...
    try {
      ...
      const orderRating =
        await this.context.orderRating.getOrderRatingByOrderSetId(orderId);
      let isBadReview = false;
      if (orderRating) {
        isBadReview = orderRating.rating < 3;
      }
      const latestStatus =
        await this.context.orderSetStatus.getLatestByOrderSet(orderId);
      const orderFulfillment =
        await this.context.orderFulfillment.getByOrderSet(orderId);
