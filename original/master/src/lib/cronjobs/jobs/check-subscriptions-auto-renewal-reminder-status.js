...
module.exports = async function CheckSubscriptionsAutoRenewalReminderStatus(jobConfig, queryContext) {
    ...
    const results = await db.raw(`
        ...
        on
            ...
            date_trunc('minute', sct.created + ((sct.remaining_minutes - ${jobConfig.AutoRenewelReminderBeforeFinishInMinutes}) * interval '1 minute')) reminder_date,
        ...
        where
            ...
            and sc.notifications ->> 'isAutoRenewalReminderSent' is null
            and date_trunc('minute'
                , sct.created + ((sct.remaining_minutes - ${jobConfig.AutoRenewelReminderBeforeFinishInMinutes}) * interval '1 minute')) = date_trunc('minute'
                , current_timestamp)
        order by
        ...
        const responses = await Promise.all(results?.rows.map(async (row) => {
            await db('subscription_customers')
                ...
                .update({
                    notifications: {
                        ...row.notifications,
                        isAutoRenewalReminderSent: true,
                    }
                });
            return axios.get(`${basePath}/c-subscription/reminder`, {
                ...
                params: {
                    ...
                    type: ReminderType.SUBSCRIPTION_AUTO_RENEWAL_REMINDER,
                },
            });
        }));
