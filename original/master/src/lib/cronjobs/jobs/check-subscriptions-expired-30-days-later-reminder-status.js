...
module.exports = async function checkSubscriptionsExpired30DaysLaterReminderStatus(queryContext) {
    ...
    const {rows: finishedSubscriptionCustomerTransactions} = await db.raw(`
        select sct.subscription_customer_id,
               ...
               date_trunc('day', sct.created + (43200 * interval '1 minute')) reminder_date,
        ...
        where sct.action_type = 'FINISHED'
          ...
          and date_trunc('day'
                  , sct.created + (43200 * interval '1 minute')) = date_trunc('day'
                  , current_timestamp)
    `)
        ...
        await Promise.all(results.map((subscriptionCustomerId) => {
            return axios.get(`${basePath}/c-subscription/reminder`, {
                ...
                params: {
                    ...
                    type: ReminderType.SUBSCRIPTION_EXPIRED_30_DAYS_LATER_REMINDER,
                },
            });
        }));
    }
