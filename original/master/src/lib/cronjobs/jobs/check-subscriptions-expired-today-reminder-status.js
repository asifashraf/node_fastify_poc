...
module.exports = async function checkSubscriptionsExpiredTodayReminderStatus(queryContext) {
    ...
    const {rows: finishedSubscriptionCustomerTransactions} = await db.raw(`
        ...
        where sct.action_type = 'FINISHED'
          ...
          and date_trunc('minute'
                  , sct.created + (30 * interval '1 minute')) = date_trunc('minute'
                  , current_timestamp)
    `)
        ...
        await Promise.all(results.map((subscriptionCustomerId) => {
            return axios.get(`${basePath}/c-subscription/reminder`, {
                ...
                params: {
                    ...
                    type: ReminderType.SUBSCRIPTION_EXPIRED_TODAY_REMINDER,
                },
            });
        }));
    }
