...
class ComputeInvoice {
  constructor(context, input) {
    ...
    //Stamp Reward
    this.isStampRewardClaimable = false;
    this.stampRewardClaimableItems = null;
    this.stampRewardRight = 0;
    this.stampRewardAmount = 0;
    this.consumedStampRewardItem = null;
  }
  ...
  async init() {
    ...
    this.isStampRewardClaimable = await this.context.stampRewardCustomer.customerHasStampReward(this.customerId);
  ...
  async getReward() {
    ...
    const brandReward = await this.context.reward.checkRewardByBrandId(this.brandLocation.brandId);
    if (brandReward && !brandReward.main) {
      this.reward = await this.context.reward
        .selectFields(this.selectFields.reward)
        .where('id', brandReward.rewardId)
      ...
      this.reward = await this.context.reward
        .selectFields(this.selectFields.reward)
        .where('brand_id', this.brandLocation.brandId)
        .where('status', rewardStatus.ACTIVE)
        .orderBy('created', 'desc')
        .then(first);
    }
  ...
  async calculatePrice() {
    ...
    this.stampRewardAmount = this.getMoney(0).round();
    if(this.isStampRewardClaimable && this.stampRewardClaimableItems.length > 0){
      if(stampRewardConfig.priceOperatorToApply == 'HIGHEST'){
        this.items.sort((a, b) => b.price.intValue - a.price.intValue);
      }else {
        this.items.sort((a, b) => a.price.intValue - b.price.intValue);
      }
      
      this.items.map(item => {
        const itemQuantityAfterCoupon = item.quantity - item.freeBySubscription - item.freeByReward - item.freeByCoupon;
        if(this.stampRewardClaimableItems.includes(item.itemId) && this.stampRewardRight > 0 && itemQuantityAfterCoupon > 0){
          this.consumedStampRewardItem = item;
          item.freeByStampRewards = 1;
          this.stampRewardRight = 0;
          this.stampRewardAmount = this.getMoney(item.price).round();
        }
      })
      if(this.stampRewardRight > 0){
        this.stampRewardRight = 0;
        this.stampRewardClaimableItems = [];
        this.isStampRewardClaimable = false;
      }
    }
    this.items.sort((a, b) => a.price.intValue - b.price.intValue);
    ...
    // sum of stamp reward free drink
    this.firstIntermediateSubtotal = this.firstIntermediateSubtotal.sub(this.stampRewardAmount);
    ...
    if (this.coupon.isCashbackCoupon) {
      subtotalWithAllDiscounts = this.subtotal.sub(this.subscriptionAmount.round()).sub(this.rewardAmount.round()).sub(this.stampRewardAmount.round());
    } else {
      ...
      subtotalWithAllDiscounts = this.getMoney(Math.max(
        this.subtotal.value - this.subscriptionAmount.round().value - this.rewardAmount.round().value - this.couponAmount.round().value - this.stampRewardAmount.round().value,
        0
      ));
    ...
    return {
      ...
      stampRewardAmount: this.stampRewardAmount.round().value,
      currency: this.currency,
  ...
  async isCouponAvailable() {
    ...
    if (
      !(await this.context.coupon.isCouponAvailableForBrandByBrandId(
  ...
  async validateOrder() {
    ...
    if(this.isStampRewardClaimable){
      const claimableItemNumber = (totalItemsCountInBasket - totalFreeDrinkCount) - this.subscriptionUsedCupsCount;
      if(claimableItemNumber > 0){
        const eligibleItems = await this.context.stampRewardEligibleItem.findStampRewardEligibleMenuItem(this.items);
        if(eligibleItems.length > 0){
          this.stampRewardClaimableItems = eligibleItems;
          this.stampRewardRight = 1;
        } else this.isStampRewardClaimable = false;
      } else this.isStampRewardClaimable = false;  
    }
  ...
  async getInvoice() {
    ...
    if (parseFloat(this.calculatedPrice.stampRewardAmount) > 0) {
      components.push({
        type: invoiceComponentType.STAMP_REWARD_DISCOUNT,
        value: this.getMoney(this.calculatedPrice.stampRewardAmount).round().value,
      });
    }
    ...
    const invoiceResult = {
      ...
      isStampRewardAvailable: this.isStampRewardClaimable,
      stampRewardValue: this.stampRewardAmount,
      rewardDetails,
      components,
      beans,
  ...
  async createOrder() {
    ...
    if (debitCreditsAndGiftCardNow) {
      ...
      // if stamp reward card was used
      if (Number(this.calculatedPrice.stampRewardAmount) > 0) {
        const stampRewardInfo = {
          item: this.consumedStampRewardItem,
          amount: this.stampRewardAmount.round().value
        }
        await this.debitStampRewardTransaction({
          customerId: orderSet.customerId,
          orderSetId,
          brandLocationId: this.brandLocation.id,
          stampRewardInfo,
          currencyId: orderSet.currencyId,
        });
      }
  ...
  debitStampRewardTransaction({
    ...
    return this.context.stampRewardCustomer.claimStampReward({
      customerId,
      orderSetId,
      brandLocationId,
      stampRewardInfo,
