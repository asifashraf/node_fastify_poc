...
const { getReferralUrl } = require('../../lib/url-shortener');
const {
  brandStatus,
  brandLocationStatus,
  statusTypes,
...
module.exports = {
  Query: {
    ...
    getCustomerMenu(root, { brandLocationId }, context) {
      if(brandLocationId){
        return context.brandLocation.calculateMenu(brandLocationId);
      }else {
        return context.brandLocation.calculateMenuForBrand();
      }
    },
    async neighborhoods(
      root,
      { countryId, countryIso, filters = { status: statusTypes.ACTIVE } },
      context
    ) {
    ...
    async countryByCode(
      ...
      // this query returns public information
      return context.country.getByCode(code, filters);
    },
    cities(
      root,
    ...
    */
    ...
    async reward(root, { id }, context) {
      return addLocalizationField(
        addLocalizationField(await context.reward.getById(id), 'title'),
        'conversionName'
      );
    },
    ...
    async categories(
      root,
      { filters = { status: statusTypes.ACTIVE }, paging },
      context
    ) {
      // this query returns public information
    ...
    /*brandSubscriptionModels(
      root,
      { paging, activeOnly, countryId, searchTerm, revenueModel, month, year },
      context
    ) {
    ...
    async getAvailableFulfillmentListForBranch(
      ...
      return context.brandLocation.getAvailableFulfilments(brandLocationId);
    },
