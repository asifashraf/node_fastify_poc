...
const otpRedis = require('ioredis');
const redisOtp = new otpRedis(otp.otpRedisConnectionUrl);
...
class OTPService {
  ...
  async validateOTP(OTPCode) {
    try {
      ...
      let operationType;
      let status = true;
      if (recordedOTP && recordedOTP === OTPCode || (bypassCode && bypassCode === OTPCode)) {
  ...
  async isRateLimitExceeded() {
    await redisOtp.incr(`${otp.OTPHitTableName}:${this.phoneNumber.number}`);

    let recorderOTPHit = await redisOtp.get(`${otp.OTPHitTableName}:${this.phoneNumber.number}`);

    recorderOTPHit = Number(recorderOTPHit);

    await redisOtp.expire(`${otp.OTPHitTableName}:${this.phoneNumber.number}`, otp.OTPHitValidityInSeconds);

    return recorderOTPHit >= otp.OTPRateLimit;
  }
    ...
    try {
      ...
      switch (OTPType) {
        case OTPTypes.SMS:
          ...
          break;
        case OTPTypes.VOICE:
          providerResponse = await provider.sendVoiceOTP(OTPCode);
          break;
        default:
          throw new OTPServiceValidationError(
  ...
  getOTPCodeFromDB() {
    return redisOtp.get(`${otp.OTPCodeTableName}:${this.phoneNumber.number}`);
  }

  getOTPBypassCodeFromDB() {
    return redisOtp.get(`${otp.OTPCodeTableName}:bypassCode`);
  }

  saveOTPCodeToDB(OTPCode, providerName) {
    return redisOtp.set(`${otp.OTPCodeTableName}:${this.phoneNumber.number}`, OTPCode, 'EX', otp.OTPValidityInSeconds);
  }

  resetOTPRateLimit() {
    return redisOtp.expire(`${otp.OTPHitTableName}:${this.phoneNumber.number}`, 1);
  }
  ...
  static async getAllOTPRecords() {
    const keys = await redisOtp.keys(`${otp.OTPCodeTableName}:*`);

    if (keys.length === 0) return [];

    const data = keys.map(async key => {
      const otpCode = await redisOtp.get(key);
      const otpTtl = await redisOtp.ttl(key);
      const [idName, identifier] = key.split(':');
      return {
        identifier,
        otpCode,
        provider: 'PROVIDER',
        ttl: Math.floor(Date.now() / 1000) + otpTtl
      }
    })
