...
module.exports = {
  Mutation: {
    ...
    async customerRequestVoiceOTPViaPhoneNumber(root, { phoneNumber, isAdmin = false }, context) {
      const res = await context.authService.checkStatus(context, phoneNumber);
      if (res) {
        return context.authService.requestPhoneVoiceOTP(phoneNumber, isAdmin);
      } else {
        return {
          status: true,
        };
    ...
    async orderSetSetStatus(root, { orderSetId, status }, context) {
    ...
    async orderSetPayWithCash(root, { orderSetId, wasPaid }, context) {
      const auth = context.auth;
      const admin = await context.admin.getByAuthoId(auth.id);
      let hasPermission = false;
      /**
      ...
      if (!hasPermission) return; // [attack_scope]
      await context.orderSet.paidWithCash(orderSetId, wasPaid);
      await publishSubscriptionEvent(
        context,
        orderSetId,
        orderSetSubscriptionEvent.ORDER_SET_UPDATED
    ...
    */
    ...
    async brandMenuItemSave(root, { menuItem }, context) {
      ...
      if (menuItem.id) {
        ...
        updateAllBranchItemStatus = !(oldMenuItem.status != menuSection.status);
      } else {
    ...
    async categorySave(root, { input }, context) {
      const categoryInput = removeLocalizationField(input, 'name');
      const result = await context.category.save(categoryInput);
      if (result.error) {
        return formatError(result, input);
      }

      const category = addLocalizationField(
        await context.category.getById(result),
        'name'
      );
    ...
    async saveBrandSubscriptionModel(root, { input }, context) {
      ...
      const id = await context.brandSubscriptionModel.addBrandSubscription(
        await context.brandSubscriptionModel.createInut(input)
      );
